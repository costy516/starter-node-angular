...
function transferProjectToRemote(failed, successful) {
  return ssh.putDirectory(
    '../start-node-angular',
    '/home/ubuntu/start-node-angular-temp',
    {
      recursive: true,
      concurrency: 1,
...
function createRemoteTempFolder() {
  return ssh.execCommand(
    'rm -rf start-node-angular-temp && mkdir start-node-angular-temp', {
      cwd: '/home/ubuntu'
  });
}
...
function updateRemoteApp() {
  return ssh.execCommand(
    'cp -r start-node-angular-temp/* start-node-angular/ && rm -rf start-node-angular-temp', {
      cwd: '/home/ubuntu'
  });
}
...
function restartRemoteServices() {
  return ssh.execCommand(
    'cd start-node-angular && sudo service mongod start && pm2 start app.js', {
      cwd: '/home/ubuntu'
  });
}
...
function sshConnect() {
  ...
  ssh
    .connect({
      ...
      host: '18.188.193.164',
      username: 'ubuntu',
      privateKey: 'D:/WozU/Module9/WozUL5.ppk'
    })
    .then(function() {
    ...
    .then(function() {
      console.log('Creating `start-node-angular-temp` folder.');
